...
Template7.registerHelper('list_object', function (a, k, options) {
	var testImg = function (str, k) {
		function ValidURL(strr) {
			return true;
		}
		if (ValidURL(str)) {
			if (str.match(/\.(jpeg|jpg|gif|png)$/) != null || k == 'avatar_url') {
				str = '<img src="' + str + '" alt="' + str + '" width="50%" />';
			}
		}
		return str;
	}
	
	var array_to_string = function (a, k, s){
		let r = (k ? '<br />' + s + '<strong>' + k + '</strong>: ' : '');
		// array or object
		if ((Array.isArray(a) || typeof a == 'object') && a) {
			for (var i in a) {
				var ss =  (Array.isArray(a[i]) || typeof a[i] == 'object') ? s + ' &nbsp; &nbsp; &nbsp;' : s;
				if (a[i]) {
					r = r + array_to_string(a[i], (typeof i == 'string' ? i : k), ss);
				}
			}
		} else {
			// verifica e trata ultimo nivel
			r = r + (typeof a == 'string' ? testImg(a, k) :  (!a ? '' : a.toString()));
		}
		return r
	};
	
	let r = array_to_string(a, (k || false), '');
	r = r.substr(6);
	return r;
});
